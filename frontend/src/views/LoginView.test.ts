...
const authMock = vi.fn((_account: string, _password: string) => Promise.resolve({ ok: false, message: '后端校验未接入' }))
vi.mock('@/lib/auth', () => ({ authenticate: (account: string, password: string) => authMock(account, password) }))
