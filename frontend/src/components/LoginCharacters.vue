...
// 眼随鼠标 + 身体微倾:瞳孔相对视口中心偏移(±5px),身体随光标横向微倾(±3deg)。
...
.lc-char { position: relative; width: 84px; height: 116px; transform: rotate(var(--tilt, 0deg)); transition: transform var(--dur-2) var(--ease); }
...
.lc-face { position: absolute; top: 22px; left: 0; right: 0; display: flex; gap: var(--sp-3); justify-content: center; }
.lc-eye { width: 20px; height: 20px; border-radius: var(--r-full); background: var(--card); display: grid; place-items: center; overflow: hidden; transition: transform var(--dur-1) var(--ease); }
.lc-pupil { width: 9px; height: 9px; border-radius: var(--r-full); background: var(--txt); transform: translate(var(--eye-x, 0), var(--eye-y, 0)); transition: transform var(--dur-1) var(--ease); }
...
.lc--account .lc-char--1 .lc-pupil, .lc--account .lc-char--2 .lc-pupil { transform: translateX(4px); }
.lc--account .lc-char--3 .lc-pupil, .lc--account .lc-char--4 .lc-pupil { transform: translateX(-4px); }
...
/* 显示密码:望向远方 + 1 号偶尔偷瞄 */
.lc--reveal .lc-pupil { transform: translateY(-4px); }
.lc--reveal .lc-char--1 .lc-pupil { animation: lc-peek 2.4s steps(1) infinite; }
@keyframes lc-peek { 0%, 80% { transform: translateY(-4px); } 85%, 95% { transform: translateY(3px); } 100% { transform: translateY(-4px); } }
