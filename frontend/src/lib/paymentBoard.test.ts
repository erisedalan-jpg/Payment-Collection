...
describe('buildPayBoardRows', () => {
  it('无区间(全部)：行字段来自 paymentPmisInRange，包含 remainingTotal', () => {
    ...
    // paymentRatio = actual/expected = 1_000_000/1_500_000
  ...
  it('区间收窄到 2024-01~2024-04：A 的节点在范围内，B 的不在', () => {
    ...
    expect(b.paymentRatio).toBeNull()
  })
...
describe('groupPayBoard（单维分桶 + 7 指标，口径收敛后 pendingSum=Σ节点未收，rate=actual/expected）', () => {
  it('全部：组1 pendingSum=ΣremainingTotal；rate=actualSum/expectedSum（非/合同）', () => {
    const rows = buildPayBoardRows(projects, pmisMap, paymentNodes, paymentRecords, '', '')
    ...
    // rate = actual/expected = 2_000_000/2_500_000 = 0.8
    expect(g1.rate).toBeCloseTo(2_000_000 / 2_500_000, 6)
    expect(g1.projectCount).toBe(2)
    expect(g1.contractSum).toBe(3_000_000)
    expect(g1.delayedNodeSum).toBe(1)
  })

  it('全部不变式：actualSum=Σ流水；pendingSum=Σ节点未收；rate=actual/expected', () => {
    const rows = buildPayBoardRows(projects, pmisMap, paymentNodes, paymentRecords, '', '')
    ...
    // 各组 rate = actual/expected（不是 actual/contract）
    const g1 = all.find((x) => x.key === '组1')!
    expect(g1.rate).toBeCloseTo(g1.actualSum / g1.expectedSum, 6)
  })
  ...
  it('区间收窄后：B 指标清零，组1 的 pendingSum/rate 仅含 A', () => {
    const rows = buildPayBoardRows(projects, pmisMap, paymentNodes, paymentRecords, '2024-01-01', '2024-04-30')
    const g = groupPayBoard(rows, ['dept'])
    const g1 = g.find((x) => x.key === '组1')!
    // B 区间内无节点无流水 → expectedTotal=0，actualTotal=0，remainingTotal=0
    // 组1: actual=1_000_000(A流水), expected=1_500_000(A节点), remaining=500_000(A未收)
    expect(g1.actualSum).toBe(1_000_000)
    expect(g1.expectedSum).toBe(1_500_000)
    expect(g1.pendingSum).toBe(500_000)
    expect(g1.rate).toBeCloseTo(1_000_000 / 1_500_000, 6)
  })
...
describe('payBoardCross / payBoardPivot', () => {
  it('cross 返回 rows/cols/cells/index', () => {
    const rows = buildPayBoardRows(projects, pmisMap, paymentNodes, paymentRecords, '', '')
    const m = payBoardCross(rows, 'dept', 'progress', 'contractSum')
  ...
  it('rate 指标空桶单元格为 NaN（展示层显 -）', () => {
    const rows = buildPayBoardRows(projects, pmisMap, paymentNodes, paymentRecords, '', '')
    const m = payBoardCross(rows, 'dept', 'progress', 'rate')
