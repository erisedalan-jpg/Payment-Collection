...
describe('riskReasons — 组合：多类同时命中时顺序', () => {
  it('顺序为 回款延期→里程碑滞后→总成本超支→交付成本超支→风险未闭环', () => {
    ...
    expect(categories).toContain('总成本超支小于5000')
    ...
    expect(categories.indexOf('里程碑滞后')).toBeLessThan(categories.indexOf('总成本超支小于5000'))
    expect(categories.indexOf('总成本超支小于5000')).toBeLessThan(categories.indexOf('交付成本超支'))
