...
- [x] **项目清单选列+表头筛选+横滚（2026-06-18，V1.9.0，feat/list-column-prefs，合并SHA待回填）**：/projects 在建 与 /projects/closed 已关闭两清单：列枚举筛选移入表头(复用 ColumnFilter)、新增选列菜单 ColumnPicker(显隐+上下排序+localStorage 持久化,useColumnPrefs)、在建增项目状态列(回款完成率与健康度间)+回款状态列、服务组(L4)→L4组、DataTable 加 fixed 列、横向滚动。
