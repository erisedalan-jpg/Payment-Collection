...
- 当前版本：**V1.5.0**
- 最近更新：2026-06-15（2D 跟进记录重调：跟进记录去云回写纯本地——删 write_followup/异步/syncStatus/sync-status；入口迁项目清单（/projects 行内跟进按钮 + FollowupModal 复用 FollowupRecords，/project:id 记录区保留）；删 /followup 整页与临期信号链 + 废 fuData）
- 维护语言：简体中文
