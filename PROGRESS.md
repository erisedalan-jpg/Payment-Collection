...
- 当前版本：**V1.4.0**
- 最近更新：2026-06-15（2C 项目标签体系：本地多标签替代纳管——preprocess 白名单播种 tagSeed、server 本地 project_tags.json 首次播种本地为准、projectTags store + /project:id 编辑 + /projects 标签列筛选 + /data 标签库管理与按标签全局排除，过滤链 naguan→exclude）
- 维护语言：简体中文
...
- [ ] **L-22（2C 记录）** /projects 项目清单导出（`lib/exportXlsx.exportRows`，SheetJS 已就绪）支持勾选列含「标签」列导出——2C 标签体系已落地，清单导出功能未建，待清单导出专项时一并支持标签列。
