...
- 上一版本：**V2.6.4**（Z 级·下钻页局部·**/insight/board 回款完成率区间口径缺陷修复，已完成**）——用户报 /insight/board 下钻完成率与实际不符（京津服务组 `QAGD-XX-REDACTED` 下钻显 0%、进详情页 70%）。**根因（systematic-debugging + 真实数据定位）**：看板下钻行 `paymentRatio` / 分组 `rate` 的分子 `actualTotal` 走日期区间过滤、分母 `contract` 全额静态——**区间内流水 ÷ 全额合同**的口径错配；叠加 FilterBar 默认区间=本年度（2026），把全部在 2025 年回款的项目虚显为 0%（实测 520 有合同项目中 **199 个≈38%** 被默认区间清零）。与全站统一口径「Σ流水净额（全加）÷Σ合同」及项目详情页 `payment.paymentRatio`（全时）矛盾。**修复（用户选 A：完成率恒全时）**：`lib/paymentBoard.ts` 行级新增 `actualAll`=Σ全时流水（`actualInRange(records,'','')`，对齐 `overview.ts` computeKpis）、`paymentRatio` 与 `deriveProgress` 改用 `actualAll/contract`（全时）；`buildGroup` 的 `actualSum`（→rate）改用 `actualAll`。**计划回款/延期/未收等仍随区间**（仅完成率脱钩区间）。**纯前端、零后端/零 schema/零依赖→升级不需点「更新数据」、无新页/无新 pageKey**。TDD（先改测试跑红再实现，含 QAGD-XX-REDACTED 真实场景回归用例）；`verify.sh` 全绿；真机冒烟：默认本年度区间下钻京津服务组，`QAGD-XX-REDACTED` 完成率现显 **70%**（原 0%）、回款状态同步显「部分回款」、0 console 报错。**同类隐患已一并修复**：`lib/paymentPmis.ts`（/payment/projects 完成率列 + /payment 总览 L4「回款数据」表 已回款/回款额完成率）与 `lib/payDashboard.ts`（/payment 总览 KPI 卡 已回款/完成率）原用同一区间分子/全额（或 S8 区间自洢）口径，默认本年度下同样虚显低值——一并改为**已回款/完成率恒全时**（全站统一口径 Σ流水全加÷Σ合同）。**为让同页 KPI 与 L4 表、清单口径自洽**，把总览 KPI（payDashboard payDashSummary）从「本区间达成率(S8)」也改为全时，与首页 computeKpis 对齐（消除首页全时 vs /payment 总览区间的既存不一致）。日期区间继续作用于 计划回款/待回款/延期/节点计数。真机冒烟：/payment/projects 该项目 已回款 52.5万+完成率 70%；京津服务组 L4 行 已回款896.09万/回款额完成率51.1%（896.09÷1752.81 自洽）；总览 KPI 完成率 50.9%（全时）。改动：`lib/paymentBoard.ts`、`lib/paymentPmis.ts`、`lib/payDashboard.ts`（各含测试）、`version.ts`。**（注：S8「本区间达成率」被全站统一口径覆盖；如仍需本区间达成视图请单独提。）已回款 KPI 同步改累计（全时），日期区间不再压低已回款/完成率数字。**
...
- **V2.6.4**（2026-07-02，**/insight/board 回款完成率区间口径缺陷修复**；bug 排查=systematic-debugging，主循环直修，非 SDD）：
  - **症状**：`/insight/board`（BoardView）分组表「完成率」列与下钻弹窗（BoardDrilldownModal）「完成率」列，对某些项目显 0%，但进 `/project/:id` 详情页显真实值（用户例：京津服务组 `QAGD-XX-REDACTED` → 下钻 0% / 详情 70%）。
  ...
  - **质量**：TDD（先改 `paymentBoard.test.ts` 把编码了旧 bug 的 3 处断言改成全时预期 + 新增 QAGD-XX-REDACTED 真实场景回归用例，跑红→改实现→26/26 绿）；`verify.sh` 全绿；真机冒烟（puppeteer+系统 Chrome，默认本年度区间）下钻京津服务组 → `QAGD-XX-REDACTED` 完成率 **70%**（原 0%）+ 回款状态「部分回款」+ 0 console 报错。**纯前端零后端/零 schema/零依赖→升级不需点更新数据、无新页/无新 pageKey**。改动：`lib/paymentBoard.ts`(+`paymentBoard.test.ts`)、`version.ts`、PROGRESS。
