...
- **V2.5.3**（2026-06-30，Z 级·页内局部调整·侧栏版式统一 + 首页成本超支去重）：
  ...
  - **纯前端、零口径/零后端/零依赖→升级不需点「更新数据」、无新页/无新 pageKey**。`verify.sh` 全绿（pytest434+ruff+typecheck+vitest976+build）；真机冒烟确认侧栏六分区子项对齐、成本超支卡=87、零 console JS 错误。改动：`riskClassify.ts`/`riskClassify.test.ts`/`AppSidebar.vue`/`AppSidebar.test.ts`/`version.ts`。**未打包/未部署/源码未提交。**
