...
- 最近更新：2026-07-02（V2.6.4：/insight/board 回款完成率区间口径缺陷修复——看板下钻/分组完成率原为「区间内流水÷全额合同」+默认区间本年度→2025 年回款项目虚显 0%（约 38%）；修为完成率恒全时（actualAll=Σ全时流水），计划/延期仍随区间。TDD+真机冒烟 QAGD-SS-202506279091 现显 70%。同类隐患 paymentPmis.ts[/payment 系列]未修，待用户决定）
...
- **V2.6.4**（2026-07-02，**/insight/board 回款完成率区间口径缺陷修复**；bug 排查=systematic-debugging，主循环直修，非 SDD）：
  ...
  - **遗留（同类隐患，未修，待用户决定）**：`lib/paymentPmis.ts` `buildProjectRows`(行 `paymentRatio`) 与 `summaryByDim`(`rate`) 用同一 `paymentPmisInRange().paymentRatio` 区间分子/全额分母模式，喂 `/payment` 总览、`/payment/projects`「完成率」列、`/payment/nodes`——默认本年度下同样虚显 0%。本次按「一次只修一处、不扩改动面」未动；如需一并按 A 口径修复请示知。
