...
- 最近更新：2026-07-01（V2.5.7：列筛选搜索交互修复（搜索态只作用搜索结果,无需先取消全选,9页）+ 售前剩余预算/已核算改为随毛利超支额（剩余=−overspendAmount 超支即负、已核算=总预算+超支额,消除"剩余≥0但超支")。纯前端 ColumnFilter.vue+costAnalysis.ts,已提交 master 249d709+**已生产上线 2026-07-01**（在线基线 V2.5.5→V2.5.7 累积含 V2.5.6）)
