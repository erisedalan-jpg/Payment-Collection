...
- **V2.5.8**（2026-07-01，Z 级·页内局部调整·跨页点击卡顿性能优化，全局 + costdetail/milestone/risk 三页）：
  ...
  - **纯前端、零后端/零 schema/零 preprocess/零依赖→升级不需点「更新数据」、无新页/无新 pageKey**。`verify.sh` 全绿（pytest434+ruff+typecheck+vitest[+`useDeferredMount` 新测]+build）；受影响单测（ChartBox/三视图/data/useDeferredMount 共 42 例）全绿。**已 git 提交（master `870953b`，10 文件）+ 更新包/手册已出待自部署（`release/pmplatform-update-V2.5.8.zip` 38 文件 1.44MB gitignore，从 V2.5.7 单版增量；`deploy/升级手册-V2.5.8.md` committed），在线现 V2.5.7。** 改动文件：`stores/data.ts`、`charts/ChartBox.vue`、`lib/useDeferredMount.ts`(+`.test.ts`)、`views/{CostDetailView,MilestoneView,RiskFollowupView}.vue`、`version.ts`、`PROGRESS.md`。性能优化手法（CPU profile 定位、shallowRef、图表懒加载、延迟渲染）以后可复用。
