...
- **V2.6.3**（2026-07-02，回款域重构 **P4 收官：五页分页 + /risk 两列筛选**；SDD 子代理驱动，特性分支 `feat/v2_6_3-pagination-risk-filter`，基线 `58593a8`；plan `docs/superpowers/plans/2026-07-02-v2_6_3-list-pagination-and-risk-filter.md`）：
  ...
  - **⭐ 本轮(P2-P4)收官——待合 master 后统一出累积升级包 V2.6.3**（从**在线基线 V2.5.9** 增量，累积含 V2.6.0/1/2/3 全部改动；升级手册须写明:含 P2 **新 pageKey `payment-key` 需给普通管理员授权** + **server.py 有改动→部署须重启服务进程** + 删三页旧链 redirect + 抽屉加宽影响 board/日历 + 统一标签筛选 vs 全局排除两套作用域;仍**无需点更新数据**）。
