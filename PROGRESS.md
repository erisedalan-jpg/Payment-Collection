...
- 最近更新：2026-07-01（V2.5.8：跨页点击卡顿性能优化——CPU profile 定位根因=挂载时同步渲染重型组件树（大 el-table 无虚拟化 + ECharts 挂载即初始化 + 16MB 数据深响应式），非业务计算。三改：data 改 shallowRef（全局）+ ChartBox IntersectionObserver 懒渲染 + 三页 useDeferredMount 延迟渲染。实测点击后骨架 13/16/30ms 出现（旧 607/218/625ms 冻结，4x 降速下 costdetail 旧 5494ms 巨块消除）。纯前端零后端，verify 全绿，**尚未提交/打包**）
...
- **V2.5.8**（2026-07-01，Z 级·页内局部调整·跨页点击卡顿性能优化，全局 + costdetail/milestone/risk 三页）：
  ...
  - **纯前端、零后端/零 schema/零 preprocess/零依赖→升级不需点「更新数据」、无新页/无新 pageKey**。`verify.sh` 全绿（pytest434+ruff+typecheck+vitest[+`useDeferredMount` 新测]+build）；受影响单测（ChartBox/三视图/data/useDeferredMount 共 42 例）全绿。**尚未 git 提交、尚未打包（待用户）。** 改动文件：`stores/data.ts`、`charts/ChartBox.vue`、`lib/useDeferredMount.ts`(+`.test.ts`)、`views/{CostDetailView,MilestoneView,RiskFollowupView}.vue`、`version.ts`、`PROGRESS.md`。性能优化手法（CPU profile 定位、shallowRef、图表懒加载、延迟渲染）以后可复用。
