...
- 最近更新：2026-07-01（V2.5.7：列筛选搜索交互修复（搜索态只作用搜索结果,无需先取消全选,9页）+ 售前剩余预算/已核算改为随毛利超支额（剩余=−overspendAmount 超支即负、已核算=总预算+超支额,消除"剩余≥0但超支")。纯前端 ColumnFilter.vue+costAnalysis.ts,未提交/未打包,在线现 V2.5.5,累积待发 V2.5.6+V2.5.7）
...
- **V2.5.7**（2026-07-01，Z 级·页内局部调整·列筛选搜索交互修复 + 售前剩余预算/已核算展示修正）：
  ...
  - **纯前端、零后端/零依赖→升级不需点「更新数据」、无新页/无新 pageKey**（overspendAmount 为现有字段）。`verify.sh` 全绿（pytest434+ruff+typecheck+vitest 全绿+build）；live 冒烟 V2.5.7 两处均生效、0 console 错误。**未提交/未打包，在线现 V2.5.5**（累积待发 V2.5.6+V2.5.7）。列筛选行为改 `ColumnFilter.vue` 一处全 9 页生效；售前预算展示口径改 `costAnalysis.ts` buildCostRows 售前分支。
