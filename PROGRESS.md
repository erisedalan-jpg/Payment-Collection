...
- 当前版本：**V1.6.0**
- 最近更新：2026-06-16（2E 人工数据导入导出 + 快照回滚：/projects 按勾选范围导出多 sheet xlsx（项目清单/标签/跟进/回款节点/里程碑，遵循当前筛选，标签顿号连接）；/data 导入「项目标签」「跟进记录」固定格式两 sheet，强校验（表头/必填/枚举/未知项目编号/长度，失败报错明细表、整体不写），导入前自动轻量快照可回滚；PMIS 取数只读不可导入；关闭 L-22）
- 上一版本：V1.5.0（2026-06-15，2D 跟进记录重调：去云回写纯本地——删 write_followup/异步/syncStatus/sync-status；入口迁项目清单（/projects 行内跟进按钮 + FollowupModal 复用 FollowupRecords，/project:id 记录区保留）；删 /followup 整页与临期信号链 + 废 fuData）
- 维护语言：简体中文
