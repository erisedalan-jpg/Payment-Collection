...
- 当前版本：**V7.8.0**
- 最近更新：2026-06-12（R2 详情页三 tab：里程碑双层三色/预算核算科目树/回款数据流水，V7.8.0）
- 维护语言：简体中文
...
- [~] **Phase R 数据源扩展批次**（spec：2026-06-12-R-batch-data-expansion-design.md，R1-R4 分期）：R1 已合并（V7.7.0，合并提交 a11aceb）；R2 详情页三 tab（进度里程碑双层三色/预算核算科目树+桥接/回款数据流水）已完成，分支 feat/phase-r2-detail-tabs 待合并（见 Handoff）。下一期 R3（数据管理页重设计+默认链接+新入口）；其后 R4（产品改名「项目管理平台」+关于页+V1.0.0 版本策略）。
