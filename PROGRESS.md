...
- 当前版本：**V2.2.2**（子页面·局部调整：风险子页面全列展示+换行；商机页放开普通管理员写[仅本人L4]）
- 最近更新：2026-06-26（V2.2.2：/project/:id 风险子页面全 43 列展示+长文本换行；/opportunities 普通管理员可编辑+新增[限本人L4，后端 can_access_l4 校验]）
- 上一版本：V2.2.1（2026-06-26，/project/:id 回款明细三列改元 + 商机新增是否重大POC列；/opportunities/key 筛选/可选列自动派生新列）
