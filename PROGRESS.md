...
- 当前版本：**V1.6.3**
- 最近更新：2026-06-17（详情页消费两列新数据：源起用户重调 input 下 5 个 CSV 导出列，经精读+全管线证实代码引用列皆在、列变更全追加式、解析已对齐；本期消费 collection_stages「收款条件」→ 回款 tab 阶段表换行列、payment_records「票据_*」→ 回款数据 tab 流水「票据」列（有才显示）。详见进行中「详情页消费两列新数据」）
- 上一版本：V1.6.2（2026-06-16，3A 回款换源「collection_stages.csv」：详情页「回款（系统核心口径）」改由收款阶段台账驱动取代 PMIS 里程碑反推、status 5 态、详情页脱离 rawNodes）
- 维护语言：简体中文
