...
- 当前版本：**V1.6.1**
- 最近更新：2026-06-16（历史快照体积优化「分组留存」：`data_history` 把全量快照拆为 JSON 产出组（analysis_data+events+snapshots，版本化留 5 份）+ Excel 源组（input 刷新到共享 `_source/` 只留最新 1 份）；`yundocs_data` 不再归档（云同步不动）；回滚只还原 JSON 产出、不动 live 源，向后兼容旧全量布局目录；`list_versions` 增 `source`、/data 数据历史卡显示「保留最近 5 份 + 源仅留 1 份」说明。体积 ~228MB(3 版)→~125MB(5 版)，可回滚版本数 3→5）
- 上一版本：V1.6.0（2026-06-16，2E 人工数据导入导出 + 快照回滚：/projects 按勾选范围导出多 sheet xlsx；/data 导入「项目标签」「跟进记录」两 sheet 强校验、导入前自动轻量快照可回滚；PMIS 只读；关闭 L-22）
- 维护语言：简体中文
