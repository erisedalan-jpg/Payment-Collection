...
- 最近更新：2026-07-02（V2.6.4：回款完成率区间口径缺陷**全站修复**——① /insight/board 看板下钻/分组完成率；② 同类隐患一并修：/payment/projects+总览 L4 表(paymentPmis.ts)、/payment 总览 KPI(payDashboard.ts,废 S8 区间自洢改全时)。全部改为**已回款/完成率恒全时**(全站统一口径 Σ流水全加÷合同)，与首页/详情页对齐；计划/待回款/延期/节点仍随区间。默认本年度下原虚显 0%/低值(约 38% 项目)已修。TDD+真机冒烟(该项目 board 70%、/payment/projects 已回款52.5万/70%、L4 自洽)。线上已 V2.6.3，基于 V2.6.3 出 V2.6.4 更新包）
