...
- 当前版本：**V1.6.2**
- 最近更新：2026-06-16（3A 回款换源「collection_stages.csv」：详情页「回款（系统核心口径）」改由 `input/collection_stages.csv`（收款阶段台账 575 项目，含此前 PMIS 缺的 合同约定日期收款/阶段验收款/质保金 全 9 类）驱动，取代 PMIS 里程碑反推；status 5 态 已回款/部分回款/质保期/延期/待回款；详情页脱离 rawNodes（删旧口径回款节点表与回款里程碑表）。详见进行中「全局下线 rawNodes 程序 ①3A」）
- 上一版本：V1.6.1（2026-06-16，历史快照体积优化「分组留存」：JSON 产出版本化留 5 份 + Excel 源共享 `_source/` 留 1 份、弃归档 yundocs，回滚仅还原产出；~228MB→~125MB、版本数 3→5）
- 维护语言：简体中文
