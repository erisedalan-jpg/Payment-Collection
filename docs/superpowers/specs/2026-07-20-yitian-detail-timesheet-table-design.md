...
### 16.4 趋势页 trend 取舍（**待 review 确认**）

trend 是纯图表页、无主表格，每个时间点的 `onTrendClick` 已占用（分流去 analytics/compliance）。"新增不破坏"在此需要一个新可点元素，但趋势图无天然第二触发点。

- **方案 A（推荐，间接可达）**：trend 不单独加明细入口；用户经 trend → analytics/compliance（现有）→ detail（本次为这两页新增的「明细」列）两跳到达。理由：时间维度直钻逐条记录量大、价值相对低，且间接路径已通。
- **方案 B（直接）**：每张趋势图标题栏加「明细」小按钮，点后进入"选周期"态，再点时间点去 `detail(dStart/dEnd)`。多一个交互态、成本较高。

推荐 A。请你 review 时定夺是否接受 trend 走间接，或坚持 B。
