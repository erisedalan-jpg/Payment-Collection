...
- **无回款阶段项目的特殊态**（如 QAGD-XX-REDACTED，`paymentNodes[pid]` 空）：抽屉内容判断——若该项目无收款阶段节点，正文显示「该项目无回款阶段数据」，并给一个**「查看完整详情」按钮** → `router.push('/project/${id}')`（与有数据项目的全页详情一致）。有节点项目维持现有抽屉内容。
