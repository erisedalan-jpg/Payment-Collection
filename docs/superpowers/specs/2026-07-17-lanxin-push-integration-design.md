...
```jsonc
{
  ...
  "credentials": {
    ...
    "idType": "employ_id"    // 取值合法性由后端校验。注意:切到 mobile/mail 并不能兜底
                             // 「人员编号≠工号」——花名册无手机号/邮箱列,拿不到 id_value。
                             // 该情形的既定应对见 §10.1(加列 + V4.0.1),不是改配置。
  },
...
| `employ_id` 编码一致性 | **蓝信「人员编号」是否等于我方「工号」（如 `A000701`）未经证实**。这是全清单里**唯一一个「答错了就得改代码」**的点 —— 其余都是填空 | 自检第②步即验证此项。**若不相等 → 见下方「§10.1 编码不一致的既定应对」** |
| `orgId` 从哪取 | 应用详情页不一定直接展示 | 申请清单已列；可由任意 staffId（`524288-xxx`）横杠前段反推 |

### 10.1 编码不一致的既定应对（用户已拍板）

**不是切 `idType` 到 `mobile`/`mail`** —— 那条路今天是断的：`input/组织架构.xlsx` 的全部列经实测为
`工号 · 姓名 · 员工类别 · 新L2组织 · 新L3组织 · 新L3-1组织 · 新L4组织 · 直接上级工号 · 直接上级姓名`，
**没有手机号、没有邮箱**。切了 `idType` 也拿不到 `id_value` 去查。

既定应对：

1. 自检第②步失败（工号换不到 staffId）即判定编码不一致；
2. **由用户更新一版 `组织架构.xlsx`，增加一列「人员编号」**（蓝信侧的人员唯一编号）；
3. 验证后如仍有问题，**在 V4.0.0 基础上出 V4.0.1 解决** —— 改动面预计仅限：
   `lanxin_recipients.read_org_tree` 多读一列落为 `lanxinId`；
   `lanxin.dispatch` 的 `id_mapping` 入参由 `工号` 改为 `lanxinId or 工号`（回退保证加列前后都能跑）。
   **不影响树结构、收件链、卡片、配置模型** —— 汇报线仍走 `直接上级工号`，与蓝信编码无关。

**V4.0.0 按「人员编号 == 工号」实现，不预埋 `lanxinId`**（YAGNI：未证实的分支不写）。
`credentials.idType` 仍保留为配置项（取值合法、后端已校验），但**其兜底能力依赖花名册补列，属代码 + 数据源变更，不是纯配置**——
这一点必须在文档与 UI 提示中说清，避免给出「编码对不上也能靠配置绕过」的错觉。
