...
原以为根因是「内网收不到公网回调」。但督办在 `10.248.107.234`、master 在 `10.248.105.95`，**同一内网网段，而督办的回调是通的**。可达性不是默认根因，必须实测定位。

但两条通道所需的可达性**方向不同**，这正是 H5 稳的原因：

| 通道 | 谁发起 | 需要什么可达性 |
|---|---|---|
| 回调 | **蓝信服务器** → 我方 | 蓝信服务器能进 `10.248.105.95` |
| H5 | **用户手机**（蓝信内置 webview）→ 我方 | 用户在公司网内即可 |

督办回调通只证明蓝信服务器能进 `10.248.107.234`；master 在**另一台主机**，防火墙策略未必相同。**H5 对这个差异完全不敏感。**
...
问题①的根因需生产实证。现状排查能力不足：新鲜度拒绝只写 `logger.warning`，生产上无人查 `journalctl`。
...
| 不做 | 理由 |
|---|---|
| **approveCard 按钮交互** | 要机器人能力二次审批 + 回调必通，门槛最高、收益与 H5 重叠。副本 README §12 自己就建议「先用 H5，等回调通了再升级」 |
...
- 问题①的**根因仍未确定**，一期只让它**可见**。拿到 `lastTimestampSample` 后按 4.4 的判定表处置，可能需要一个补丁版本修 `lanxin_timestamp_fresh`。
