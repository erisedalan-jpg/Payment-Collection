...
### 2.3.1 【2026-07-29 追加】地址修复后的第二次取证：根因链第三环

超管把回调地址改为 `http://10.248.105.95/pm/api/lanxin/callback` 后，`rejected.count` 从 0 变为 **2**（后续增至 4），`lastReason` = **`stale`**。

**这一步一次性排除了三件悬案**（闸门顺序 ② 验签 → ②b 新鲜度，能走到 `stale` 说明验签**已通过**）：

| 悬案 | 结论 |
|---|---|
| `callbackSignToken` 是否配对 | **配对**（否则 `lastReason` 会是 `signature`） |
| **公网可达性**（§2.4② 的担心：蓝信在公网、`10.248.105.95` 是私网） | **可达，实证排除**。这是唯一需要跨部门网络申请的一条，就此消解 |
| 事件类型订阅（`sendAs=bot` 需 `bot_private_message`） | **已订阅**，否则蓝信根本不会推 |

**第三环根因：`timestamp` 是 19 位纳秒。** 生产日志（`stale` 分支那行 `logger.warning`，只打 timestamp、不打签名/密钥/报文体）打出四条原值：

| timestamp 原值 | ÷1e9 得秒 | 换算 CST | 日志时刻 | 误差 |
|---|---|---|---|---|
...
四条逐条对上、误差 0~1 秒 → **单位是纳秒，且两端时钟正常**（服务器 `date -u +%s` = 1785297084，与最后一条差 173 秒，落在 ±300 窗口内）。末尾 `608457348` 这类是真实亚秒精度，蓝信后端多半是 Go 的 `time.Now().UnixNano()`。

现有 `lanxin_timestamp_fresh` 的 `if abs(val) >= 10**11: val //= 1000` 只除**一次**（按秒/毫秒两种量纲写的），19 位纳秒除一次只到微秒，与 now 差 1.78e15 秒 → **每一条回调都判 stale**。

**处置**：`if` → `while` 逐级降到秒，阈值 `10**11` 不动（真实的秒值要到公元 5138 年才会达到它）。见实施计划 Task 0。

**完整根因链（三环，全部实证）**：

```
① 回调地址漏 /pm 前缀 → 请求打到同机别的系统   【已修:改蓝信后台,非代码】
② 公网可达/验签/事件订阅                        【实证排除,非问题】
③ timestamp 19 位纳秒,代码只除一次 → 全判 stale 【V4.5.8 Task 0 修复】
```

**下一关尚未验证**：解密（AES-256-CBC）与解析（两套键名）两道闸门**从未实测过** —— ③ 修完会第一次走到它们。上线后须实测。

### 2.4 两条被取证推翻/更正的既有判断

**① 「timestamp 格式假设」的定位一改再改，最终证实。** 初稿把它列为「最可能的单点根因」→ 第一次取证否定（`lastReason` 空串 = 新鲜度闸门一次都没执行）→ 修好地址后的第二次取证**证实它确实是根因，只是排在地址之后**。教训：闸门式流水线里，**下游闸门的沉默不代表它没问题，只代表流量没到它**。
...
1. **【2026-07-30 更正】员工看到的正是该工号「最近一次 `primary` 推送」的那几项**——不严格等于「他手上那张卡」本身。`token` payload 只有 `{emp, kind}`，不带推送批次标识，`items`/`submit` 都是按 `emp`+`kind`+`role=='primary'` 取【最近一次】同类推送的快照（见 `handle_lanxin_review_items`）。48 小时 TTL 内若发生第二次推送，员工点开旧卡片打开的会是【新】快照；若新快照移除了某项目，按旧卡提交会被 ③ 越权写闸拒绝（返回「该条目不在你的待办清单内」——fail-safe，但文案容易被误读成权限问题，而非快照已更新）。此取舍已知且影响温和，本期不改实现，见 `PROGRESS.md` L-61。
...
| ~~本期修改 `lanxin_timestamp_fresh` 解析逻辑~~ | **此条已作废并移入本期（Task 0）**。当时的理由是「拿到实证前改它是猜测」；2026-07-29 拿到四条真实报文后前提消失，改它不再是猜测而是照实证办事 |
...
8. **回调打通**（本期新增，因 Task 0 并入）：上线并 `systemctl restart` 后，让一名员工回复推送卡片，**存证文件 `data/lanxin_callback_raw.jsonl` 应首次出现**，且 `rejected.count` 不再因 `stale` 增长。

> **①②两环已在上线前完成**（零代码）：回调地址已改为带 `/pm` 前缀；公网可达性、验签、事件订阅均经 §2.3.1 实证排除。本期 Task 0 修第三环。

**上线后必须实测的下一关**：存证出现之后，还有**解密（AES-256-CBC）**与**解析（两套键名）**两道从未跑过的闸门。若收件箱出现 `status: "unparsed"` 条目，即卡在这两关 —— `unparsedReason` 会写明是哪一道，报文原文在存证文件里可查。**这不是缺陷而是设计**：看不懂的报文落「未解析」保留原始数据，绝不静默丢弃。
...
- 问题①的**三环根因已全部实证**（§2.3 + §2.3.1）：① 地址漏 `/pm`（已修，非代码）；② 可达性/验签/订阅（排除）；③ timestamp 纳秒（本期 Task 0）。
- **旧债「回调 `timestamp` 单位/格式是未证实假设」已可勾销** —— 实证为 19 位纳秒，四条真实样本钉在 `tests/test_server_lanxin_callback.py` 的 `REAL_NS_SAMPLES`。
- **解密与解析两道闸门仍从未实测**，Task 0 修完才会第一次走到。上线后按 §8 第 8 条实测。
- `lastTimestampSample`（§4.4.1）虽然本次靠日志就拿到了原值，**仍然要做** —— 靠 `journalctl` 排查需要 root、需要知道 grep 什么，下一个人未必有这两样。
