...
回复文本**一律 HTML 转义**，绝不当富文本解析 ——
回复本来就是纯文本（`msgType: text`），没有任何理由给它富文本能力。

**换行只能用 `<br>`，不能用 `<p>`。** `frontend/src/lib/richText.ts:2` 的读端白名单是
`B / STRONG / U / I / EM / S / STRIKE / DEL / BR / SPAN / FONT` —— `<p>` **不在其中**，
未白名单的标签会被「拆解、保留净化后子内容」，写 `<p>` 等于白写。
