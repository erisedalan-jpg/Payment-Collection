...
**凭证到位后的验收（本版无法自验，交付时一并交代给用户）**：填入四项凭证 → 点「连通性自检」→ 三步全绿即完成。
若**第②步「工号换人员ID」失败**：先核 `orgId`；`orgId` 无误则说明**蓝信「人员编号」≠ 我方工号** ——
按 spec §10.1 走「用户给 `组织架构.xlsx` 加一列人员编号 → 出 V4.0.1」，**不要试图靠切 `idType` 绕过**（花名册无手机号/邮箱，切了也无值可查）。
...
| §9 不做清单 | 计划中无对应任务（正确） |
| §10 待实测清单 | Task 3（`56008` 退避）+ Task 2（`fields` 主动不越限）+ Task 9 Step 4（自检验编码一致性） |
| §10.1 编码不一致的既定应对 | **本版不实现**（YAGNI：未证实的分支不写）。V4.0.0 按「人员编号 == 工号」实现；若自检第②步失败 → 用户给花名册加「人员编号」列 → 另出 V4.0.1（改动面仅 `read_org_tree` 多读一列 + `dispatch` 的 `id_mapping` 入参回退）。Task 1 的 `_ID_TYPES` 注释已写明「切 `idType` 不能兜底」，避免误导 |
