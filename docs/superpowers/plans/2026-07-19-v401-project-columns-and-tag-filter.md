...
**一处例外**：T5 除自己的视图文件外，还会改 `frontend/src/lib/projectList.ts` 删除
`ProjectFilters.tags`（见 T5 Step 5）。T3 与 T5 分属不同波次、串行执行，不构成并行冲突；
之所以不放在 T3，是因为删类型字段与停止写入必须同一提交，否则中间态过不了 typecheck。
...
**Files:**
- Modify: `frontend/src/lib/projectList.ts:7-36`（`ProjectRow`）、`:59-95`（`buildProjectRows`）
...
**本任务只加字段，不删任何东西。** `ProjectFilters.tags` 的清理归 Task 5。
...
Expected: vitest PASS，typecheck **0 error**。

**本任务只做加法，不动 `ProjectFilters.tags`。** 标签筛选的死代码清理归 Task 5——那里同时改视图层，两处一起改才能保证每个提交都能通过 typecheck。若在此任务里先删类型字段，`ProjectsView.vue` 仍在给 `sp.tags` 赋值，会留下一个不通过 typecheck 的中间提交。

- [ ] **Step 6: 提交**

```bash
git add frontend/src/lib/projectList.ts frontend/src/lib/projectList.test.ts
git commit -m "feat(projectList): 三个日期字段
...
- Modify: `frontend/src/lib/projectList.ts`（`ProjectFilters.tags` 死代码清理，见 Step 5）
- Test: `frontend/src/views/ProjectsView.test.ts`、`frontend/src/lib/projectList.test.ts`

**Interfaces:**
- Consumes（T3 产出）：`ProjectRow.originSetupDate` / `.plannedFinalAcceptDate` / `.actualFinalAcceptDate`
- Produces: 无

**为什么本任务同时改 `projectList.ts`：** 视图层停止写入 `sp.tags` 与类型层删除 `ProjectFilters.tags` 必须在**同一个提交**里完成，否则中间态无法通过 typecheck。Task 3 已按此约定只做加法。
...
- [ ] **Step 5: 移除工具栏标签筛选 + 清理 `ProjectFilters.tags`**

这两处必须一起改、一起提交，否则中间态过不了 typecheck。

**视图层** `frontend/src/views/ProjectsView.vue`：

1. 模板中删除这一行：`<TagFilterSelect v-model="sp.tags" />`
2. `<script setup>` 中删除：`import TagFilterSelect from '@/components/TagFilterSelect.vue'`
3. `sp` 的初始化对象中删除 `tags: []` 一项
4. 若 `defineExpose` 需要扩展（见 Step 1），一并改为 `defineExpose({ ALL_COLUMNS, FILTERABLE, prefs })`

**类型层** `frontend/src/lib/projectList.ts`（此时 `sp.tags` 已无写入方）：

5. `ProjectFilters` 接口删除 `tags: string[]` 一行。
6. `applyProjectFilters` 删除这一行：
   ```ts
   if (f.tags && f.tags.length && !tagMatch(r.tags ?? [], f.tags)) return false
   ```
7. 文件顶部删除 `import { tagMatch } from './tagFilter'`。
8. 删除 `projectList.test.ts` 中断言 `f.tags` 过滤行为的用例（若有）。
...
Run: `cd frontend && npx vitest run src/views/ProjectsView.test.ts src/lib/projectList.test.ts && npm run typecheck`
Expected: vitest PASS；typecheck **0 error**。

若 typecheck 报出 `ProjectFilters.tags` 在**其他文件**仍被消费，停下来报告——这与调研结论（只有本页写入）不符，不要自行给其他文件打补丁。

- [ ] **Step 7: 提交**

```bash
git add frontend/src/views/ProjectsView.vue frontend/src/views/ProjectsView.test.ts frontend/src/lib/projectList.ts frontend/src/lib/projectList.test.ts
