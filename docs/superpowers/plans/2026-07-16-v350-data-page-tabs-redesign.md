...
### Task 2: tabs 骨架 + 信息架构归属调整

**Files:**
- Modify: `frontend/src/views/DataView.vue`（模板 `279-453` 的 `.dv-domain-grid` 整块；样式 `519-526`）
- Modify: `frontend/src/views/DataView.test.ts`（新增 tabs 断言）

**Interfaces:**
- Consumes: Task 1 无（本任务不接分发函数）
- Produces: 三个 `el-tab-pane`（`name="sources" | "config" | "maint"`），供 Task 3–7 往里填抽好的组件
...
删除现 519–526 行的 `.dv-domain-grid` 规则块，替换为：
...
git commit -m "refactor(data): /data 立 tabs 骨架(数据源/配置/维护)+归属调整,废除 auto-fit 改显式两栏"
```
...
.dv-more :deep(.el-collapse-item__header) { font-size: var(--fs-2); font-weight: 700; color: var(--txt); padding-left: var(--sp-4); }
.dv-more :deep(.el-collapse-item__content) { padding-bottom: var(--sp-2); }
</style>
```
...
.dv-maint { margin: 0; background: transparent; border: none; box-shadow: none; }
