...
## ⚠ Task 2 执行后的既成事实（Task 3/5/6/7 必读）

Task 2 抽走共享样式后，DataView 仍内联着 5 张卡（要到 Task 3–7 才逐张抽走），因此**这些卡的特有 CSS 被临时留在 `DataView.vue` 的 `<style scoped>` 里**（Task 2 原稿漏了它们，执行时已按「页面观感不得变化」的要求补回）：

| 临时寄存在 DataView 的规则 | 归属组件 | 由哪个任务带走 |
|---|---|---|
| `.dv-cookie` / `.dv-cookie-box` | MainDomainSourceCard | **Task 3** |
| `.dv-fmt` / `.dv-fmt code` | YitianSourceCard | **Task 5** |
| `.dv-tags-mgr` / `.dv-tag` / `.dv-tag.off` / `.dv-tag-name` | ProjectTagsCard | **Task 6** |
| `.dv-err` / `.dv-err th, .dv-err td` / `.dv-danger-title` | MaintenanceCard | **Task 7** |

**每张卡抽成组件时，必须把上表对应行的规则从 `DataView.vue` 的 `<style scoped>` 中删除**（它们随组件走，已写在各组件的 style 块里）。漏删 = DataView 攒下死样式。
Task 7 完成后，`DataView.vue` 的 `<style scoped>` 应只剩：`@import`、`.data-view`、`.dv-top`、`.dv-title`、`.dv-primary`(+`.dv-card-head` 覆写)、`.dv-pane-grid`、`.dv-span-all`、`.dv-tabs :deep(...)` 与窄屏 media query。

**另记（Task 2 实测更正）**：`:deep()` 编译后 scope 属性落在**祖先选择器**上 —— `.dv-more :deep(.el-collapse-item__header)` → `.dv-more[data-v-521656a9] .el-collapse-item__header`，而非挂在 deep 目标上。计划原稿 Step 8 的第一条 grep 形状写错，已实测更正；判断 `:deep` 是否生效应查祖先形式，或直接用「产物无字面 `:deep(` 残留」这一条。共享 `dataview.css` 里的 `:deep` 确认拿到了 DataView 的 scope（hash `521656a9` 与 `.data-view[data-v-521656a9]` 一致）。
