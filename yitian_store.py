...
本库是**服务端私有**(含工时ID 等去重/私有字段,供规则变更后重新判定),累积库不整库下发前端;
下发给前端的是 data/yitian_data.json(码表压缩)。工作成果 V4.1.3 起随明细页整列下发(用户 2026-07-21
授权),此前按隐私裁列、仅问题行带 120 字摘要(snippet 仍保留供问题原因 tooltip)。
"""
