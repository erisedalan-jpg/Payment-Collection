...
// 只把「回款延期」设为已启用,其余 code 保持 enabled:false ——
// 这样才能同时覆盖「已启用项渲染」与「未启用项仍渲染」两种场景。
// LTS 只有 project 一条路由(无倚天,故无 timesheet 路由)。
const CFG = {
...
describe('LanxinConfigCard', () => {
  ...
  it('选项源是全集,不是已勾选的子集(否则取消勾选后再也勾不回来)', async () => {
    const w = await mountCard()
    // 配置里 project 只勾了「回款延期」1 个 reason,但表格必须给出全部 8 个(items 恒为完整白名单长度)
    const html = w.html()
    expect(html).toContain('回款延期')
    expect(html).toContain('里程碑滞后')         // 未启用,但必须仍在表格里
  })
})

describe('V4.0.2 逐项配置', () => {
  it('路由渲染成逐项表格,行数 = 白名单全集', async () => {
    const w = await mountCard()
    const rows = w.findAll('[data-test="lx-item-row"]')
    expect(rows.length).toBe(8) // 项目关注原因 8 类(LTS 只有 project 一条路由)
  })
  ...
  it('未启用的项仍然渲染 —— 否则取消启用后就再也开不回来', async () => {
    // CFG 里「里程碑滞后」未启用(只有「回款延期」启用),该行仍须出现在 DOM 里。
    const w = await mountCard()
    expect(w.html()).toContain('里程碑滞后')
  })
  ...
  it('后端若返回不完整 items,UI 不崩且如实渲染收到的项', async () => {
    ...
    partial.routes[0].items = [
      { code: '回款延期', enabled: true, primary: true, supervisorLevels: 0 },
    ]
    vi.mocked(getLanxinConfigFull).mockResolvedValueOnce({ config: partial })
    const w = await mountCard()
    // 只剩 1 行 —— 如实渲染收到的,不崩也不吞
    expect(w.findAll('[data-test="lx-item-row"]').length).toBe(1)
    expect(w.html()).toContain('回款延期')
  })
})
...
describe('V4.0.5 回调双向配置(发送身份/回调双凭证/回调地址/拒绝计数)', () => {
  ...
  // Important-2：验签失败与时间戳新鲜度失败共用同一个计数器，超管必须能从文案分清
  // 该查 signToken 还是查时间戳格式/两端时钟——否则会去查一堆本来就是对的东西。
  it('lastReason 为 stale 时提示时间戳/时钟问题，不误导去查签名', async () => {
    const wrapper = await mountCard({}, {
      rejected: { count: 3, lastAt: '2026-07-20 10:00:00', lastReason: 'stale' },
    })
    expect(wrapper.text()).toContain('时间戳')
    expect(wrapper.text()).not.toContain('签名令牌填错了')
  })

  it('lastReason 为 signature（或缺省）时提示签名令牌填错，不提时间戳', async () => {
    const wrapper = await mountCard({}, {
      rejected: { count: 3, lastAt: '2026-07-20 10:00:00', lastReason: 'signature' },
    })
    expect(wrapper.text()).toContain('签名令牌填错了')
  })
