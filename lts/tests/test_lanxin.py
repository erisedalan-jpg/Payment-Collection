...
# ── build_plan / dispatch ──────────────────────────────────────────────
# LTS 无倚天工时,build_plan 只处理 kind=="project"(master 版另有 timesheet 分支,已剥除)。
...
# B2 新增用例用的别名:_TREE/_PMIS 复用既有夹具;
...
def _cfg(project_levels=1, project_on=True):
...
def _cfg_items(pj_items=None, pj_on=True):
    """新结构配置工厂。pj_items: {code: (enabled, primary, levels)}，未列出的 code 补 (False, True, 0)。"""
...
def test_build_plan_behavior_equivalence_after_migration_golden():
    ...
    旧配置里 recipients.primary/supervisorLevels 是【整条路由统一】的,对 reasons 里
    勾选的每一项都套用同一组 primary/levels —— 这正是 lanxin_config._migrate_route_items
    的定义(见 lanxin_config.py:122-131),所以"手工套用旧语义算期望值"与"迁移后按逐项
    items 跑 build_plan"在数学上必然给出同一个答案,除非迁移或 build_plan 哪里出错,
    故此golden值可作为等价性的可执行证据。
    ...
    (LTS 移植说明:master 版此用例同时覆盖 project+timesheet 两条路由;
    LTS 无倚天工时域,本用例只保留 project 路由部分,断言逻辑不变。)
    """
    legacy = _cfg(project_levels=2, project_on=True)
    ...
    expected = [
        ...
        {"employId": "A007", "role": "primary",
         "bodyTitle": "你名下 1 个项目存在关注原因",
         "fields": [{"key": "回款延期", "value": "1 个项目"}]},
        # ② 项目 supervisor:+2 于岩(A002,看耿磊磊子树合计2)先、+1 耿磊磊(A005,看张三/李四)后
...
def test_build_plan_recipients_carry_route_and_project_ids():
    """归因候选依赖这两个键;缺了会导致归因候选下拉恒为空(LTS 虽无归入功能,
    但 routeKey/projectIds 仍是 sentLog 反查、发送台账溯源的基础字段,不可丢)。"""
...
# ── I-1 回归:reply_hint 必须由 build_plan 真正接线到卡片 ──────────────────
#
# 为什么这一组【必须打 build_plan 的产物】而不是打 builder:
# 上面的字节上限测试直接调 LR.build_project_card(..., reply_hint=True) 并手动传 True,
# 于是 build_plan 一次都没传过 reply_hint(生产调用方 0 处)也照样全绿 ——
# 定义处、直调测试处都有,生产 0 处,员工收到的卡片底部从来没有过引导语,
...
def _project_card_kinds_cfg():
    """一次同时产出两种卡片:项目 primary / 项目汇总。
    supervisorLevels=1 让 A006(张三)卷到 A005(耿磊磊)。"""
    return _cfg_items(pj_items={"回款延期": (True, True, 1)})
...
def test_build_plan_adds_reply_hint_when_callback_credentials_configured():
    ...
    cfg = _cfg_with_callback(_project_card_kinds_cfg())
    plan = LX.build_plan(_PROJECT_KIND_ITEMS, cfg, TREE, PMIS)

    assert len(plan["recipients"]) == 2, "夹具应同时覆盖 primary/supervisor 两种卡片"
    for r in plan["recipients"]:
...
def test_build_plan_omits_reply_hint_when_callback_not_configured():
    ...
    cfg = _project_card_kinds_cfg()          # default_config 里两个回调凭证均为空串
    plan = LX.build_plan(_PROJECT_KIND_ITEMS, cfg, TREE, PMIS)
...
def test_build_plan_omits_reply_hint_when_only_one_credential_configured():
    ...
    for aes, token in (("A" * 43, ""), ("", "tok-abc")):
        cfg = _cfg_with_callback(_project_card_kinds_cfg(), aes=aes, token=token)
        plan = LX.build_plan(_PROJECT_KIND_ITEMS, cfg, TREE, PMIS)
        assert LR.REPLY_HINT not in _plan_cards_json(plan), \
...
def test_build_plan_treats_whitespace_credentials_as_unconfigured():
    ...
    cfg = _cfg_with_callback(_project_card_kinds_cfg(), aes="   ", token="  ")
    plan = LX.build_plan(_PROJECT_KIND_ITEMS, cfg, TREE, PMIS)
    assert LR.REPLY_HINT not in _plan_cards_json(plan)
