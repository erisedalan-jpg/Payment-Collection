"""I-4(移植自 master):/api/lanxin/{preview,send} 此前没有 except,凭证错/花名册
缺失时异常直穿 socketserver、连接直接断开,前端只看到 Failed to fetch。本文件用真实
HTTP 请求验证两个端点把这些异常转成结构化的 400 响应。

LTS 只保留 project 路由(无倚天工时,故只有 kind="project" 事项),与 master 版
(project + timesheet 双路由)不同。
"""
import json
import http.client
import threading
...
def test_preview_missing_org_file_degrades_gracefully_not_400(tmp_path, monkeypatch):
    ...
    read_org_tree 因此优雅降级成空树({byId:{},byName:{}}),预览照常 200(项目经理姓名
    在空树里查不到,落 unresolved)。这条测试锁住这个事实。"""
    ...
    monkeypatch.setattr(server, "_load_analysis_cached", lambda: {
        "projectPmis": {"P0001": {"projectName": "测试项目", "team": {"项目经理": "张三"}}},
    })
    try:
        conn, cookie = _login(port)
        items = [{"kind": "project", "projectId": "P0001", "reasons": ["回款延期"]}]
        ...
        assert body["plan"]["unresolved"][0]["reason"] == "经理不在花名册"
    finally:
        srv.shutdown()
        srv.server_close()
...
def test_send_returns_400_when_another_send_already_in_progress(tmp_path, monkeypatch):
    """推送不可撤销,双击或两个超管同时点会重复触达全员 —— 服务端并发锁必须挡住第二次调用。
...
def test_preview_malformed_project_id_returns_400_not_disconnect(tmp_path, monkeypatch):
    """畸形/被篡改的前端请求(projectId 传成不可哈希的类型)会让 project_pmis.get(pid)
    抛 TypeError,必须转 400,而不是让异常直穿把连接断掉。"""
    srv, port = _srv(tmp_path, monkeypatch)
    monkeypatch.setattr(lanxin_recipients, "read_org_tree",
                        lambda path: {"byId": {}, "byName": {}})
    try:
        conn, cookie = _login(port)
        items = [{"kind": "project", "projectId": ["不可哈希"], "reasons": ["回款延期"]}]
        conn.request("POST", "/api/lanxin/preview", json.dumps({"items": items}),
                     {"Content-Type": "application/json", "Cookie": cookie})
        r = conn.getresponse()
        assert r.status == 400
