...
- **排版严格层级**：六级 `--fs-1..6`(12/14/16/19/25/34 @16基准)，每级字号·字重·色锁定，不混用。**图标字号 `--fs-icon-sm`(10)/`--fs-icon-lg`(24) 不属于六级层级**，仅用于图标与 emoji，不得承载正文。**卡内小标题固定 `--fs-3` / 600 / `--txt`**（现状 `.gov-h` 用 700、`.yt-h` 用 600，以多数派 600 为准）。
