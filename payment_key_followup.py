"""回款重点跟进(/payment/key)领域纯函数:范围条件规整 + 进展编辑/归档。
可单测,不依赖 server。范围匹配在前端做(数据已按 L4 裁剪),本模块只规整与存储进展。
分组 scope(同 temp:project/paymentNode/milestone 三组);归档不清 current(跟进留存,同 risk)。"""
...
def apply_archive(store, rows, now) -> None:
    """只追加历史快照;不清空 current(跟进动作/复盘结论/下次复盘时间 留存,同 risk)。"""
